...
class BaseLogger:
    ...
    async def get_log_channel(self, guild: discord.Guild, event_type: str) -> Optional[discord.TextChannel]:
        """
        Smart channel resolution with multiple fallback levels:
        1. Try event-specific channel from log_event_channels table
        2. Fall back to guild's default log_channel_id
        3. Return None if no channels configured
        """
        ...
        try:
            ...
            # Level 2: Fall back to default guild log channel
            ...
            # Level 3: No channels configured
            logger.warning(f"No log channel configured for {event_type} in guild {guild.name}")
            return None
