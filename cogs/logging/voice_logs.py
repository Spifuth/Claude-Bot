...
class VoiceLogs(LoggingModule):
    ...
    async def cleanup_stale_sessions(self):
        """Clean up stale voice sessions (in case we missed a leave event)"""
        try:
            current_time = datetime.utcnow()
            stale_sessions = []

            for user_id, session in self.active_sessions.items():
                # Sessions older than 6 hours are considered stale
                if current_time - session['start_time'] > timedelta(hours=6):
                    stale_sessions.append(user_id)

            for user_id in stale_sessions:
                logger.warning(f"Cleaning up stale voice session for user {user_id}")
                await self.end_session(user_id, "Stale session cleanup")
